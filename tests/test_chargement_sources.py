...
class TestChargement(unittest.TestCase):
    def test_normalize_title(self):
        a = chargement_sources.normalize_txt("['Ambiance(s). Ville, architecture, paysages. [coord. du dossier, N° 113 de Culture et Recherche]']")
        b = chargement_sources.normalize_txt("['Rayonnement solaire et environnement urbain : de l’héliotropisme au désenchantement, histoire et enjeux d’une relation complexe']")
        self.assertIsInstance(a, str)
        self.assertEqual(a, 'ambiancesvillearchitecturepaysagescoorddudossiern113decultureetrecherche')
        self.assertEqual(b, 'rayonnementsolaireetenvironnementurbaindelheliotropismeaudesenchantementhistoireetenjeuxdunerelationcomplexe')
    ...
    def test_chargement_hal(self):
        api_to_csv(donnees["data"]["dois"]["hal_fichier_api"], donnees["api_hal"])
        hal = chargement_sources.chargement_hal(donnees["data"]["dois"]["hal_fichier_api"], skip=0)
